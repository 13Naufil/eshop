(function($) {
  $(".cstm-pro-swatches-btn a").click(function(){
    var test = this;
    var id;
    var dd = $(this).parent().prev('.look-color-swatch').find('.active').attr('data-value');
    var d = $(this).parent().prev('.look-color-swatch').prev('.item-size-swatch').find('.active').attr('data-value');
    var img = $(this).parent().parent().parent().children('a').children('img').attr('src');
    var text = $(this).parent().parent().parent().children('a').children('h2').text();
    var com = d+" / "+dd;
    var selector = $(this).parent().parent().children('.product-selectors').html();
    ...
    ella.doAjaxAddToCart(id, '1', text, img);
  ...
  var ella = {
          ...
          if ($(this).parent().hasClass('inner')) {
            if ($(this).hasClass("mobile-toggle-open")) {
              ...
              $(this).parent().parent().children().each(function() {
                $(this).children().removeClass("mobile-toggle-open");
    ...
    initInfiniteScrolling: function() {
      if ($('.infinite-scrolling').length > 0) {
        $('.infinite-scrolling a').click(function(e) {
          ...
          setTimeout(function(){ 
            var idClass = $('.products-grid .grid-item');
            var getItem = idClass.length;
            $('#itemsCount').text(getItem);
            $('.progressTag').val(getItem);
            ella.addCartPull();
          }, 5000);
    ...
    doInfiniteScrolling: function() {
      ...
      var cstm_sold_elemt =$('.block-row .products-grid .cstm_sold_elemt');
      if (!currentList.length) {
        currentList = $('.block-row .product-list');
        var cstm_sold_elemt =$('.block-row .products-list .cstm_sold_elemt');
      }
      if (currentList) {
...
//               currentList.append(products.children());
              cstm_sold_elemt.before(products.children());
              cstm_sold_elemt.removeClass('cstm_sold_elemt');
    ...
    initDropdownSearch: function() {
      $('.nav-search .icon-search').click(function() {
        $(this).toggleClass('x');
        if ($('.header-bottom .search-bar').is(':visible')) {
          $('.header-bottom .search-bar').slideUp('fast');
        } else {
          $('.header-bottom .search-bar').slideDown('fast');
        }
      });
    },
    ...
    initCloudzoom: function() {
      if ($("#product-featured-image").length > 0) {
        if ($(".visible-phone").is(":visible")) {
          ...
          $("#zoomMobile").bind("click", function(e) {
    ...
    doAjaxAddToCart: function(variant_id, quantity, title, image) {
      $.ajax({
        ...
        error: function(xhr, text) {
          ella.hideLoading();
          $('.ajax-error-message').text("Some items are no longer available.");
          ella.showModal('.ajax-error-modal');
        }
      });
    },
        ...
        for (var i = 0; i < product.options.length; i++) {
          ...
          for (var j = 0; j < product.variants.length; j++) {
            ...
            if (optionValues.indexOf(value) < 0) {
              ...
              if (is_color) {
                options += '<label for="' + forText + '" class='+ valueHandle +' style="background-color: ' + valueHandle + '; background-image: url(' + filePath + valueHandle + '.png)"><img class="crossed-out" src="' + assetUrl + 'soldout.png" /></label>';
              } else {
    ...
    addCartPull: function() {
      ...
      $(".sizeSwatch li").on('click', function(e) {
        console.log('1');
        e.preventDefault();
        var check = false;
        ...
        var print = $('.' + PID + ' strong');
        print.text(dd);
        var d = $(this).parent().parent().next().children().children('.active').attr('data-value');
        var size = dd;
        var selector = $(this).parent().parent().prev().html();
        $(this).closest('#sizeSwatch').siblings('.cstm-pro-swatches-btnsss').children('.btn_adds').addClass('f')
        //                     debugger;
        //                    alert(selector);
        //                     $(selector).each(function() {
        //                         var avl = $(this).attr('data-aval');
        //                         var text = $(this).text().toLowerCase();

        //                         if (text.indexOf(idd) > -1) {
        //                             if (avl == 'available') {
        //                                 check = true;
        //                             }
        //                         }
        ...
        if (check) {
          $(this).parent().parent().next().next().children('a').show();
          $(this).parent().parent().next().next().children('input').hide();
        } else {
          $(this).parent().parent().next().next().children('a').hide();
          $(this).parent().parent().next().next().children('input').show();
        }
      });

      $(".colorSwatch li").on('click', function(e) {
        e.preventDefault();
        //                   	debugger;
        var check = false;
        var dd = $(this).attr('data-value');
        ...
        print.removeClass();
        print.addClass(dd);
        var d = $(this).parent().parent().parent().children().next().children().children('.active').attr('data-value')
        if(d == dd){
          $('.ajax-error-message').text("Kindly Select Size First.");
          ella.showModal('.ajax-error-modal');
        }else{

          var size = dd + " / " + d;
          var selector = $(this).parent().parent().parent().children().html();
          ...
          if (check) {
            $(this).parent().parent().parent().children().next().next().next().children('a').show();
            $(this).parent().parent().parent().children().next().next().next().children('input').hide();
          } else {
            $(this).parent().parent().parent().children().next().next().next().children('a').hide();
            $(this).parent().parent().parent().children().next().next().next().children('input').show();
          }
        }
      });
      ...
      $('.cstm-pro-swatches-btnsss a').click(function(e) {
        console.log('2');
        var test = this;
        var id= $(this).parent().prev().prev().children('.sizeSwatch').find('.active').attr('data-id');
        var dd = $(this).parent().prev().children('.colorSwatch').find('.active').attr('data-value');
        var d = $(this).parent().prev().prev().children('.sizeSwatch').find('.active').attr('data-value');
        //               debugger;  
        var img = $(this).closest('.pulllook_form').siblings('.product-image').children('a').children('img').attr('src');
        var text = $(this).parent().closest('.product-top').siblings('.product-bottom').children('.product-title').text();
        var com = d;
        //                 var selector = $(this).parent().parent().children('.product-selectors').html();
        //                 $(selector).each(function() {
        //                     var text = $(this).text().toLowerCase();
...
$(document).ready(function(){
  ...
  $( ".swatch" ).each(function( index ) {
    $( ".swatch .header:contains('Season')" ).parent().addClass('season-found');
  });
  $("#login_model").on("click", function() {
    $("#myModal_login").modal("show");
  });

  $('#add_wishlist_button').on('click',function(e) {
    e.preventDefault();
    var simProducts = $(this).attr('pid');
    var product_ids = $('#add_wishlist_').find('[name=product_id]').val();
    var customer_ids = $('#add_wishlist_').find('[name=customer_id]').val();
    if (simProducts == 'Configurable Products') {
      var variant_ids = $('#add_wishlist_').find('[name=variant_id]').val();
      if (variant_ids == "") {
        $('#myModal_selectsize').modal('show');
      }else{
        var json = 'https://simplywishlist.alchemative.net/wish_list_integrations/create_with_list';
        $.ajax({
          type: 'POST',
          url: json,
          data: { 
            ...
            shop_domain:'outfitterspk.myshopify.com'
          },
...
$(window).load(function() {
  $("#product-variants").children().find(".Color");

});
if($(document).width()>=768){

  jQuery('.cstm_notifyme_form_show').hover(function() {
    var title = jQuery(this).attr('data-title');
    var variant = jQuery(this).attr('data-variant');

    jQuery('#notify_me_form').fadeIn(500);
    jQuery('.notify_me_form_active').hide(500);
    jQuery('#notify-me-body').val("Please notify me when "+title+"("+variant+")"+" becomes available.");
   
  });

  jQuery('.cstm_notifyme_form_hide').hover(function() {
    jQuery('.notify_me_form_active').show(500);
    jQuery('#notify_me_form').fadeOut(500);
...
{
  jQuery('.cstm_notifyme_form_show').click(function() {
    var title = jQuery(this).attr('data-title');
    var variant = jQuery(this).attr('data-variant');
  ...
  jQuery('.cstm_notifyme_form_hide').click(function() {
    jQuery('.notify_me_form_active').show(500);
    jQuery('#notify_me_form').fadeOut(500);

  });
}
$('#more-view-carousel img').click(function(e){
  var attr = $(this).attr('alt');

  if(attr.indexOf('video') >= 0){
    $('.video-product').get(0).play();
    $('.video-product').show();
    $('.product-photo-container #product-featured-image').hide();
  }else{
    $('.product-photo-container #product-featured-image').show();
    $('.video-product').hide();
    $('.video-product').get(0).pause();
  }
});
$('.desk-eid .number-top').hover(
  function () {
    $('.custm_top_bar_small_popup.active').show();
  }, 
  function () {
    $('.custm_top_bar_small_popup.active').hide();
  });
    $('.sortby-btn .btn').click(function(){
      if($(this).parent().hasClass('active')){
