                                    ...
                                    if (!isset($menu['parents'][$itemId])) {
                                        ...
                                        if (count($actions_ar) > 0) {
                                            ...
                                            foreach ($actions_ar as $act) {
                                                if ($act != '') {
                                                    $actions .= '<li data-jstree=\'{ "icon" : "icon-accessibility2" }\'>';
    ...
    (function ($) {
        $(document).ready(function () {
            ...
            tree.on("changed.jstree", function (e, data) {
                if (data.action == 'deselect_node') { tree.jstree("close_node", "#" + data.node.id); }
                else { tree.jstree("open_node", "#" + data.node.id); }
            });
            ...
            $('.tree-form').on('submit', function (e) {
                $('.jstree input[type=checkbox]', this).each(function () { $(this).prop('checked', false); });
                $('.jstree-undetermined', this).each(function () { $(this).parent().find('input').prop('checked', true); });
                $('.jstree-clicked', this).each(function () { $(this).parent('li').find('input').prop('checked', true); });
            });
        });
