...
//echo '<pre style="display: none;">';print_r($this->benchmark->elapsed_time('code_start', 'code_end'));echo '</pre>';
    }
    ...
    {
        $menu_link = (!empty($menu_link) ? $menu_link : getUri(1));
        ...
        if (file_exists(get_template_directory() . $file_name) != false) {
            $this->template->load($temp_name, $data);
    ...
    function do_contact()
    {
        ...
        $contact_email = get_option('recipient_email');
        ...
        //$this->email->bcc('developer.systech@gmail.com');
        ...
        if (!$this->email->send()) {
            $mailmsg = "<p class='alert alert-danger'>Email sending faild, Please try again.</p>";
            $this->session->set_flashdata('contact_error', $mailmsg);
        } else {
