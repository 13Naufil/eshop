 ...
 * @property M_job_seekers $m_job_seekers * @copyright 2019 * @date 04-10-2019
 */
...
class Job_seekers extends CI_Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method job_seekers __construct
     * @model job_seekers main_model    | m_job_seekers
     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method job_seekers index | Grid | listing
     * *****************************************************************************************************************
     */
    ...
    public function index()
    {
        ...
        $where = '';
        replace_find_query('experience', 'job_seekers.experience', $where);
        replace_find_query('job', 'jobs.title', $where);
        replace_find_query('status', 'job_seekers.status', $where);
    ...
    public function view()
    {
        ...
        $id = intval(getUri(4));
        $query = "SELECT *,
        CONCAT('<a href=\"" . asset_url('admin/cv/') . "/',cv,'\">',cv,'</a>') as cv  FROM {$this->table} WHERE {$this->id_field}='{$id}'";
        $data['query'] = $query;

        $data['config']['buttons'] = array('new', 'update', 'delete', 'refresh', 'print', 'back');
        $data['title'] = $this->module_title;

        activity_log('view_job_seekers', $this->table, $id);
    ...
    public function add()
    {
        if (!$this->module->validate()) {
            ...
            activity_log('create_job_seekers', $this->table, $id);
    ...
    public function update()
    {
        ...
        if (!$this->module->validate()) {
            ...
            $DbArray = getDbArray($this->table);
            $DBdata = $DbArray['dbdata'];
            $DBdata['currently_empolyed'] = getVar('currently_empolyed');
            $where = $DbArray['where'];
            save($this->table, $DBdata, $where);
            activity_log('update_job_seekers', $this->table, $id);
    ...
    function delete()
    {
        ...
        activity_log('view_job_seekers', $this->table, explode(',', $id));
...
/* End of file job_seekers.php */
/* Location: ./application/controllers/admin/job_seekers.php */