 ...
 * @property M_banner_management $m_banner_management * @copyright 2019 * @date 18-09-2019
 */
...
class Banner_management extends CI_Controller
{
  ...
  , `status`
   FROM " . $this->table . " WHERE 1 " . $where;
    ...
    {
        //$this->db->query("ALTER TABLE `banner_management` ADD COLUMN `status` ENUM('Active','Inactive') DEFAULT 'Active' NOT NULL AFTER `description`; ");
        $data = array();
        $id = intval(getUri(4));
        ...
        activity_log('view_banner_management', $this->table, $id);
    ...
    public function add()
    {
        if (!$this->module->validate()) {
            ...
            if ($_FILES['mobileimage']['name']) {
                $mobileupload = $this->module->file_upload('mobileimage');

                if (!$mobileupload['status']) {
                    $this->form($this->input->post(NULL, TRUE));
                } else {
                    $mobilefile_name = $mobileupload['upload_data']['file_name'];
                }
                $DBdata['mobileimage'] = $mobilefile_name;
            }
            $DBdata['created'] = date('Y-m-d H:i:s');
            $DBdata['description'] = getVar('description', false, false);
            $id = save($this->table, $DBdata);
            activity_log('create_banner_management', $this->table, $id);
    ...
    public function update()
    {
        ...
        if (!$this->module->validate()) {
            ...
            activity_log('update_banner_management', $this->table, $id);
    ...
    function delete()
    {
        ...
        activity_log('view_banner_management', $this->table, explode(',', $id));
