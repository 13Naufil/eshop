 ...
 * @property M_attribute_groups $m_attribute_groups * @copyright 2019 * @date 20-09-2019
 */
...
class Attribute_groups extends CI_Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method attribute_groups __construct
     * @model attribute_groups main_model    | m_attribute_groups     * *****************************************************************************************************************
     */
    ...
    /**
     * *****************************************************************************************************************
     * @method attribute_groups index | Grid | listing
     * *****************************************************************************************************************
     */
    ...
    public function form($Request = '')
    {
        ...
        if (!empty($id)) {
            ...
            $data['row'] = $RS->row();
            $data['selected_attribute'] = singleColArray("SELECT * FROM attribute_group_rel WHERE attribute_group_id=" . $id, 'attribute_id');
        } elseif ($Request) {
            $data['row'] = $Request;
        }

        $data['attribute_sets'] = $this->db->get('attribute_sets')->result();
    ...
    public function view()
    {
        ...
        activity_log('view_attribute_groups', $this->table, $id);
    ...
    public function add()
    {
        if (!$this->module->validate()) {
            ...
            activity_log('create_attribute_groups', $this->table, $id);

            $attribute_group_rel = getVar('attribute_group_rel');
            if (count($attribute_group_rel) > 0) {
                foreach ($attribute_group_rel as $rel_id) {
                    save('attribute_group_rel', array('attribute_group_id' => $id, 'attribute_id' => $rel_id));
                }
            }
    ...
    public function update()
    {
        ...
        if (!$this->module->validate()) {
            ...
            activity_log('update_attribute_groups', $this->table, $id);

            delete_rows('attribute_group_rel', "attribute_group_id=" . $id);
            $attribute_group_rel = getVar('attribute_group_rel');
            if (count($attribute_group_rel) > 0) {
                foreach ($attribute_group_rel as $rel_id) {
                    save('attribute_group_rel', array('attribute_group_id' => $id, 'attribute_id' => $rel_id));
    ...
    function delete()
    {
        ...
        activity_log('view_attribute_groups', $this->table, explode(',', $id));
...
/* End of file attribute_groups.php */
/* Location: ./application/controllers/admin/attribute_groups.php */