 ...
 * @property M_brands $m_brands * @copyright 2019 * @date 19-09-2019
 */
...
class Brands extends CI_Controller
{
    ...
    public function add()
    {
        if (!$this->module->validate()) {
            ...
            if ($_FILES['logo']['name']) {
                $upload = $this->module->file_upload('logo');
                ...
                $DBdata['logo'] = $file_name;
            }
            $DBdata['friendly_url'] = url_title($DBdata['friendly_url'],'-',true);
            
            $id = save($this->table, $DBdata);
            activity_log('create_brands', $this->table, $id);
    ...
    public function update()
    {
        ...
        if (!$this->module->validate()) {
            ...
            $DbArray = getDbArray($this->table);
            $DBdata = $DbArray['dbdata'];
            //$DBdata['friendly_url'] = url_title($DBdata['title'],'-',true);
            if ($_FILES['logo']['name']) {
                $upload = $this->module->file_upload('logo');
            ...
            activity_log('update_brands', $this->table, $id);
