 ...
 * @property M_order_statuses $m_order_statuses * @copyright 2019 * @date 03-08-2019
 */
if (!defined('BASEPATH')) exit('No direct script access allowed');

class Order_statuses extends CI_Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method order_statuses __construct
     * @model order_statuses main_model    | m_order_statuses
     * *****************************************************************************************************************
     */
    ...
    public function index()
    {
        ...
        $query = "SELECT
      order_statuses.id
    , order_statuses.status_title
    , order_statuses.status_code
    , email_templates.name AS email_template
    FROM order_statuses
        LEFT JOIN email_templates ON (email_templates.id = order_statuses.email_template)
WHERE 1 " . $where;
    ...
    public function view()
    {
        ...
        activity_log('view_order_statuses', $this->table, $id);
    ...
    public function add()
    {
        if (!$this->module->validate()) {
            ...
            activity_log('create_order_statuses', $this->table, $id);
    ...
    public function update()
    {
        ...
        if (!$this->module->validate()) {
            ...
            $row = $this->db->get($this->table, array($this->id_field => $id), 1)->row();
            ...
            if($row->status_code != $DBdata['status_code']){
                save('orders', array('status' => $DBdata['status_code']), "status='{$row->status_code}'");
            }
            activity_log('update_order_statuses', $this->table, $id);
    ...
    function delete()
    {
        ...
        activity_log('view_order_statuses', $this->table, explode(',', $id));
...
/* End of file order_statuses.php */
/* Location: ./application/controllers/admin/order_statuses.php */