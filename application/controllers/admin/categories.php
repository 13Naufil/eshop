 ...
 * @property M_categories $m_categories * @copyright 2019 * @date 17-09-2019
 */
...
class Categories extends CI_Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method categories __construct
     * @model categories main_model    | m_categories     * *****************************************************************************************************************
     */
    ...
    public function index()
    {
        ...
        replace_find_query('category', 'category.title', $where);
        replace_find_query('parent', "IF(category.parent_id > 0 ,parent_category.title ,'Main')", $where);


        $query = "SELECT
                      category.id
                      , category.title as category
                      , IF(category.parent_id > 0 ,parent_category.title ,'Main') AS parent
                      , category.thumb
                      , category.include_in_menu
                      , category.status
                      , category.created
                FROM {$this->table} AS category
                LEFT JOIN {$this->table} AS parent_category ON parent_category.id=category.parent_id
                LEFT JOIN brands  ON brands.id=category.brand_id
                WHERE 1 " . $where;
    ...
    public function view()
    {
        ...
        $data = array();
        $id = intval(getUri(4));
        $query = "SELECT id, parent_id AS parent, title, thumb, include_in_menu, status, created FROM {$this->table} WHERE {$this->id_field}='{$id}'";
        $data['query'] = $query;

        $data['config']['buttons'] = array('new', 'update', 'delete', 'refresh', 'print', 'back');
        $data['title'] = $this->module_title;

        activity_log('view_categories', $this->table, $id);
    ...
    public function add()
    {
        if (!$this->module->validate()) {
            ...
            if ($_FILES['thumb']['name']) {
                $upload = $this->module->file_upload('thumb');
            ...
            if ($_FILES['icon']['name']) {
                ...
                if (!$upload['status']) {
                    $this->session->set_flashdata('error', 'error ocured in icon image');
                } else {
                    $file_name = $upload['upload_data']['file_name'];
                }
            ...
            activity_log('create_categories', $this->table, $id);

            # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            # product_ids
            $product_ids = getVar('product_ids', true, false);
            delete_rows('product_cat_rel', 'category_id=' . $id);
            if(count($product_ids) > 0){
                foreach ($product_ids as $product_id) {
                    if(!empty($product_id)) {
                        ...
                        save('product_cat_rel', $_product_data);
                    }
                }
            }
    ...
    public function update()
    {
        ...
        if (!$this->module->validate()) {
            ...
            $DBdata['friendly_url'] = url_title($DBdata['friendly_url'], '-', true);
            $DBdata['include_in_menu'] = getVar('include_in_menu');
            $where = $DbArray['where'];
            save($this->table, $DBdata, $where);
            activity_log('update_categories', $this->table, $id);
    ...
    function delete()
    {
        ...
        activity_log('view_categories', $this->table, explode(',', $id));
